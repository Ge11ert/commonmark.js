...
var blocks = {
    ...
    table_block: {
        ...
        canContain: function(t) {
          return ['table_head', 'table_body'].includes(t);
        },
        acceptsLines: true
    },
};
...
var blockStarts = [
    ...
    function (parser, container) {
        ...
        const prevLineLooksLikeTable = container.type === 'paragraph' && container._string_content.includes('|');
        const curLineLooksLikeTable = !parser.indented && line.includes('|');

        if (prevLineLooksLikeTable && curLineLooksLikeTable) {
            parser.closeUnmatchedBlocks();
            const tableBlock = new Node('table_block', container.sourcepos);
            tableBlock._string_content = container._string_content;
            container.insertAfter(tableBlock);
            container.unlink();
            parser.tip = tableBlock;
            parser.advanceNextNonspace();
            return 1;
        } else {
            return 0;
    ...
    function (parser, container) {
        ...
        if (container.type !== 'table_block') {
            return 0;
        }

        if (line === '| ------------- | ------------- |') { // TODO: replace with RegExp
            const tableHead = new Node('table_head', container.sourcepos);
            const headStringContent = container._string_content.slice(0, container._string_content.length - line.length);
            tableHead._string_content = headStringContent;
